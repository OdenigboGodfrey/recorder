...
class RandomGenerator
{
     ...
     * @return string
     */
    public static function generate_personal_code(Model $model)
    {
        $model = new $model;

        $code = self::generate_unique_uuid();

        $exists = $model->withTrashed()->whereCode($code)->first();
        if($exists){
            self::generate_staff_code();
        }
